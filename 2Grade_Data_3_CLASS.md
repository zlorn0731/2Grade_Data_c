...
### 3장 연습문제
- 1. A, B, C, D, E를 순서대로 넣었다 꺼낸 결과는?
     - 답 : E D C B A

- 2. 올바른 설명을 모두 고르시오.
     - A. 스택은 FIFO ❌ → 스택 : LIFO
     - B. 양쪽 끝을 사용하여 입출력한다 ❌ → 양쪽 끝으로 입출력 : 덱
     - C. 함수 호출 시 복귀주소 저장 🅾️
     - D. 배열 사용하여 구현 가능 🅾️

- 3. 배열로 구현된 스택의 삽입과 삭제연산은 시간복잡도는?
     - 답 : 삽입 = O(1), 삭제 = O(n)

- 4. A, B, C, D를 출력할 때, 가능한 출력순서사 아닌것은?
     - 1. D, A, B, C ❌ → 구조적으로 될 수 없음
       2. A, B, C, D 
       3. A, B, D, C
       4. B, C, D, A

- 5. 스택의 응용분야로 거리가 먼 것은?
     - A. 미로 찾기
     - B. 수식 계산 및 수식 표기법
     - C. 운영체제의 작업 스케줄링 ❌ → 여러 프로세서를 CPU는 하나씩 꺼내서 실행
     - D. 서브루틴의 복귀번지 저장
    
- 6. A*(B+C)/D-E 후위 표기법으로 표현하기
     - 답 : ABC+*D/E-

- 7. 후위 표기 수식 A B E + D * -을 수행하면 제일 처음에 수행되는 연산은?
     - 1. B+E 🅾️
       2. E+A 
       3. D*B 
       4. B*E
